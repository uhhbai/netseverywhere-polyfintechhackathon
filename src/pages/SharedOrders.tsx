...
const SharedOrders = () => {
  ...
  const handleViewReceipt = (sessionId: string, sessionName: string) => {
    const merchantName =
      sessionName.split(" ")[0] + " " + sessionName.split(" ")[1];
    navigate(
      `/receipt-details?session=${sessionId}&merchant=${encodeURIComponent(
        merchantName
      )}&from=shared-orders`
    );
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  $
                  {sharedSessions
                    .filter((s) => s.my_status === "paid")
                    .reduce((sum, s) => sum + s.my_amount, 0)
                    .toFixed(2)}
                </p>
              ...
              <Card key={session.id} className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant={getStatusColor(session.my_status)}
                      className="text-center px-3 py-1 text-sm leading-none"
                    >
                      {session.my_status === "pending" && "Items not selected"}
                      {session.my_status === "selected" &&
                        `$${session.my_amount.toFixed(2)}`}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      handleViewReceipt(session.id, session.session_name)
                    }
                  >
